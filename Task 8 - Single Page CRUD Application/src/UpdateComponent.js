...
export default function UpdateHotel(props) {
    ...
    useEffect(() => {
        var dataPromises = Axios.get("https://localhost:5001/api/Hotel/GetHotel/" + props.match.params.hid);
        dataPromises.then((response) => {
            sethid(response.data.hid);
            setdoubleprice(response.data.doublePrice);
            setdoublebed(response.data.doubleBed);
            setsingleprice(response.data.singlePrice);
            setsinglebed(response.data.singleBed);
            sethname(response.data.hName);
            sethlocation(response.data.hLocation);
            sethaddress(response.data.hAddress);
    ...
    return (
        <div style={styleObject.divstyle}>
            <h1 style={{ textAlign: "center" }}>Edit Hotel Info</h1>
            <br></br>
            <div className='h-100 d-flex justify-content-center align-items-center' >
                <Form onSubmit={handleSubmit}>
                    <Form.Group className="mb-3">
                        <Form.Label>Hotel ID</Form.Label>
                        <Form.Control style={{ width: "400px" }} type="number" placeholder="Enter Hotel ID" name="hid" value={hid} onChange={handleChange} readOnly={true}/>
                    </Form.Group>
                    <Form.Group className="mb-3">
                    ...
                    <Button variant="primary" style={{margin:"15px 40%"}} type="submit">
                        Submit
                    </Button>
                </Form>
            </div>
